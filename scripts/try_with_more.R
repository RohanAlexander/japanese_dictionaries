...
i <- read_lines("outputs/01_after_ocr/02-i.ocr.txt")
u <- read_lines("outputs/01_after_ocr/03-u.ocr.txt")
e <- read_lines("outputs/01_after_ocr/04-e.ocr.txt")
o <- read_lines("outputs/01_after_ocr/05-o.ocr.txt")


a <- tibble(raw = a, letter = "a")
i <- tibble(raw = i, letter = "i")
u <- tibble(raw = u, letter = "u")
e <- tibble(raw = e, letter = "e")
o <- tibble(raw = o, letter = "o")
...
all <- 
  ...
  mutate(raw = str_replace_all(raw, "之", "え"),
         raw = str_replace_all(raw, "乏", "え"),
         raw = str_replace_all(raw, "乏", "え"),
         raw = str_replace_all(raw, "工", "エ"),
         )
# Fix some i
all <- 
  all %>% 
  mutate(raw = str_replace_all(raw, "^■", "イ"),
  )

# Fix some u
all <- 
  all %>% 
  mutate(raw = str_replace_all(raw, "5", "う"),
         raw = str_replace_all(raw, "\\$", "う"),
  )
...
start <- sample(x = c(1:nrow(o)), size = 1)
o[start:(start+15),]
